...
This project creates a toolbar-type container with a `Dropdown` component within the
`App` at `src/App.js`.  Clicking the Button shows a `Menu` with two `Item`
components.  The `Dropdown`, `Menu`, and `Item` components are provided by the
[Semantic UI library](https://www.npmjs.com/package/semantic-ui-react).  Menu
item clicks are handled by the `handleAddUser` method on the `App` component.
...
button to show its menu.  Click on each menu item to see the handling of the
user interaction logged to the console.
