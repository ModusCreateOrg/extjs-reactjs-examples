...
This project contains a `Foo` component in `src/Foo.js`.  When instantiated, `Foo` fetches data from a remote file and calls the static `MyError.fromAjax` method from `MyError` at `src/MyError.js`.  It does so by first importing the class at the top of the `Foo` file:
