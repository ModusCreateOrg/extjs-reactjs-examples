...
In this example, two child components are composed within the parent container.
The active / visible / rendered component is determined by the index number
passed in the `activeCard` prop of the `App` component.  It defaults to 0.
...
A browser window will automatically open and will load the app.  Change the
`activeCard` prop to `1` in `src/index.js` to show the second of the two child
components.
