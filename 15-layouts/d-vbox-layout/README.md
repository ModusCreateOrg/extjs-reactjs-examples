...
In this example, four child components are composed within the parent container. Each child has a different width within the parent's flexbox style.  The items are oriented vertically instead of horizontally by setting `flex-direction: column;` on the `.layout-vbox` style rule.
